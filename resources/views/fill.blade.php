...
<body>
	...
	function renderForm(s){
		...
		s.questions.forEach(q=>{
			...
			switch(q.type){
				...
				case 'radio': { q.options.forEach(o=>{ const r=document.createElement('div'); const i=document.createElement('input'); i.type='radio'; i.name=name; i.value=o.id; const l=document.createElement('label'); l.textContent=o.label; r.appendChild(i); r.appendChild(l); b.appendChild(r); }); break; }
				case 'checkbox': { q.options.forEach(o=>{ const r=document.createElement('div'); const i=document.createElement('input'); i.type='checkbox'; i.name=name+'[]'; i.value=o.id; const l=document.createElement('label'); l.textContent=o.label; r.appendChild(i); r.appendChild(l); b.appendChild(r); }); break; }
				case 'dropdown': { const s1=document.createElement('select'); s1.name=name; q.options.forEach(o=>{ const opt=document.createElement('option'); opt.value=o.id; opt.textContent=o.label; s1.appendChild(opt); }); b.appendChild(s1); break; }
	...
	document.getElementById('submitBtn').addEventListener('click', async (e)=>{
		...
		if(res.ok){ alert('Thanks! Response submitted.'); formEl.reset(); }
